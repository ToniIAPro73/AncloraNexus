    ...
    <div className={className}>
        ...
        <div className="rounded-lg p-3 mb-4 cursor-pointer" style={{ backgroundColor: validationResult.valid ? 'rgba(40, 167, 69, 0.1)' : 'rgba(220, 53, 69, 0.1)' }}>
          <div className="flex items-center" style={{ gap: 'var(--space-2)' }}>
            ...
            <span className="font-medium" style={{ color: validationResult.valid ? 'var(--color-success)' : 'var(--color-danger)' }}>
              {validationResult.valid ? 'Archivo válido' : 'Problemas detectados'}
            </span>
          </div>
      ...
      <div className="grid gap-6" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))' }}>
        <div className="rounded-lg p-4" style={{ backgroundColor: 'var(--color-neutral-100)', border: `1px solid var(--color-neutral-200)` }}>
          <h4 className="text-h4 font-semibold mb-4" style={{ color: 'var(--color-neutral-900)' }}>Información básica</h4>
          <Field label="Título" icon={<IconEbook className="w-4 h-4 text-primary" />} value={data.title} onChange={(v) => setData({ ...data, title: v })} editing={isEditing} />
          ...
          <Field label="Descripción" value={data.description} onChange={(v) => setData({ ...data, description: v })} editing={isEditing} />
          <Field label="Formato" value={data.format?.toUpperCase()} editing={false} />
        </div>

        <div className="rounded-lg p-4" style={{ backgroundColor: 'var(--color-neutral-100)', border: `1px solid var(--color-neutral-200)` }}>
          <h4 className="text-h4 font-semibold mb-4" style={{ color: 'var(--color-neutral-900)' }}>Detalles de publicación</h4>
          <Field label="Editorial" value={data.publisher} onChange={(v) => setData({ ...data, publisher: v })} editing={isEditing} />
          <Field label="Fecha de publicación" icon={<IconCalendar className="w-4 h-4 text-primary" />} value={data.publishDate} onChange={(v) => setData({ ...data, publishDate: v })} editing={isEditing} />
