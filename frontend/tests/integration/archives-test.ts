...
describe('Conversiones de Archivos Comprimidos', () => {
    ...
    it('debe extraer archivos especÃ­ficos', async () => {
      ...
      const result = await converter.extract({
        ...
        options: {
          ...
          filter: {
            include: ['documents/report.pdf', 'images/logo.png']
          }
        }
      });
  ...
  describe('Operaciones de compresiÃ³n', () => {
    ...
    it('debe crear archivo desde lista de archivos', async () => {
      ...
      const tempDir = path.join(process.cwd(), 'temp', 'compress-files', Date.now().toString());
  ...
  describe('Manejo de archivos multi-volumen', () => {
    ...
    it('debe dividir archivo grande', async () => {
      ...
      result.parts.forEach((part: string) => tempFiles.push(part));
    });
