...
def purchase_credits():
    """Compra créditos adicionales (simulado para desarrollo)"""
    try:
        user_id = get_jwt_identity()
        user = User.query.get(user_id)
...
@jwt_required()
def gift_credits():
    """Regala créditos a otro usuario (función administrativa)"""
    try:
        user_id = get_jwt_identity()
        admin_user = User.query.get(user_id)
        
        # Solo administradores pueden regalar créditos
        # En un entorno real, verificarías roles/permisos
        ...
        transaction = CreditTransaction(
            ...
            description=f'Créditos regalo de {admin_user.email}'
        )
...
def upgrade_plan():
    ...
    try:
        ...
        # Si es un upgrade, añadir créditos del nuevo plan
...
@jwt_required()
def get_usage_stats():
    """Obtiene estadísticas detalladas de uso de créditos"""
    try:
        user_id = get_jwt_identity()
        user = User.query.get(user_id)
        ...
        # Créditos usados por día (últimos 30 días)
        ...
        # Créditos por tipo de conversión
...
def get_credit_packages():
    """Obtiene los paquetes de créditos disponibles para compra"""
    packages = [
        {
            'credits': 50,
            'price': 4.99,
