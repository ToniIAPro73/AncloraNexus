...
try:
    ...
    logging.warning("PyMuPDF no disponible para PDF→PNG de alta calidad")
...
try:
    ...
    logging.warning("pdf2image no disponible para PDF→PNG")
...
def convert(input_path, output_path):
    """Convierte PDF a PNG usando la mejor librería disponible"""
    ...
    if PYMUPDF_AVAILABLE:
        try:
            ...
            if success:
                return True, f"Conversión PDF→PNG exitosa con PyMuPDF - {message}"
    ...
    if PDF2IMAGE_AVAILABLE:
        try:
            ...
            if success:
                return True, f"Conversión PDF→PNG exitosa con pdf2image - {message}"
        except Exception as e:
...
def convert_with_pymupdf(input_path, output_path):
    ...
    try:
        ...
        if os.path.exists(output_path) and os.path.getsize(output_path) > 0:
            return True, f"Primera página convertida: {pix.width}x{pix.height}px (144 DPI)"
        else:
...
def convert_with_pdf2image(input_path, output_path):
    ...
    try:
        ...
        if os.path.exists(output_path) and os.path.getsize(output_path) > 0:
            ...
            return True, f"Primera página convertida: {width}x{height}px (150 DPI)"
        else:
            return False, "Error: PNG no se generó correctamente"
...
def convert_with_pypdf_fallback(input_path, output_path):
    ...
    try:
        ...
        # Título
        draw.text((40, 40), "PDF → PNG", fill='black')
        ...
        return True, f"PNG básico generado: 800x600px con información del PDF ({len(pdf_reader.pages)} páginas)"
...
def convert_basic_fallback(input_path, output_path):
    ...
    try:
        ...
        draw.text((20, 30), "PDF → PNG", fill='black')
        ...
        return True, f"PNG placeholder generado: 400x300px (instalar PyMuPDF para conversión real)"
